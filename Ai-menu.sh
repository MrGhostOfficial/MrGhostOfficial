...
while true; do
    ...
    case "$choice" in
        1)
            ...
            echo -e "${green}[+] Starting ComfyUI with optimized CPU threads...${finished}"
            echo -e "${yellow}[-] Stop server CTRL+C${finished}"
            # Determine number of CPU cores (fallback if nproc not available)
            if command -v nproc &>/dev/null; then
                CPU_CORES=$(nproc)
            else
                CPU_CORES=$(grep -c ^processor /proc/cpuinfo 2>/dev/null || echo 1)
            fi
            OMP_NUM_THREADS=$CPU_CORES python main.py --cpu --listen 0.0.0.0
            ;;
        2)
            echo
